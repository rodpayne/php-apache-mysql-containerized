...
#			$app_info = Get_Configuration_Item('HS-'.$requestString);
	...
	if (!$app_info_string) {
		echo "Error: Failed to retrieve info from HS provider. \n";
		exit;
	}
...
//-----------------------------------------------------------------------------
	...
	if (isset($_SERVER["HTTP_REFERER"]) && strpos($_SERVER["HTTP_REFERER"], 'hs-device-service.php') === FALSE) {
		Exit($app_info_string);
	} else {
	#	Format for human who browsed directly:	
        print "<a href='/service/hs-device-service.php?request=getstatus'>getstatus</a><br>";
        print "<a href='/service/hs-device-service.php?request=getcontrol'>getcontrol</a><br>";
        print "<a href='/service/hs-device-service.php?request=getevents'>getevents</a><br>";
        print "<a href='/service/hs-device-service.php?request=getlocations'>getlocations</a><br>";
		print "<hr>";
