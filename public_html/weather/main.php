...
<?php include("../include/header_dropdown_menu.php"); ?>
			<table id='currentInfo' class='info shadow border'>
				<tr>
					<td class='vat'><canvas id='currentIcon' width='128' height='128' style='width:128;height:128'></canvas></td>
					<td align='center'><div id='summary'><div class='summary'> <span id='currentTemp'></span>˚ <span id='currentSummary'></span></div>
					<span id='hourlySummary'></span><br></div>
					</td>
				</tr><tr>
					<td colspan='2'>
					...
					<td colspan='2' align='center'>
						...
						<div class='details' id='dailySummary'></div>
		...
		<button id="refresh-map" onclick="var iframe = document.getElementById('embedded-map'); iframe.src = iframe.src;">
			<!-- Refresh icon by Icons8 --><img width="20" height="20" src="data:image/png;base64,">
		</button>
		</td></tr>
	</table>
...
function RetrieveWeatherData() {
	...
	/* Format Current Conditions */
			...
			strDailySummary = strDailySummary.replace(", ",",<br>");
...
function WeatherOnPageLoad() {
	...
	var weatherTimer = setInterval(RetrieveWeatherData,60000);	
}
...
function WeatherActualOnResize() {
	...
	if (innerWidth < 600) {									/* not enough room to show the map */
		...
		if (iframe.src == "about:blank") {
			if ( typeof weatherAppInfo !== 'undefined' && weatherAppInfo ) {
				...
				iframe.src = 'https://maps.darksky.net/@precipitation_rate,'+weatherSelectedLatitude+','+weatherSelectedLongitude+',9?marker='+actualLatitude+','+actualLongitude+'&linkto=maps';
			}
		}
	...
	if ( typeof weatherAppInfo !== 'undefined' && weatherAppInfo ) {
		...
		if (!PrecipitationAnyMinute) {
			...
			console.log('currentInfo'+document.getElementById("currentInfo").width+', leftWidth='+leftWidth+', minutelyCanvasWidth='+minutelyCanvasWidth);
			document.getElementById("minutelyForecast").innerHTML = "<table style='padding:0; margin:0;' width='100%' class='details'><tr><td class='text-left'>"+formatDate(new Date(weatherAppInfo.minutely.data[0].time * 1000),"h:mm tt")+"</td><td class='center-text'>Forecast for the next hour</td><td class='right-text'>"+formatDate(new Date(weatherAppInfo.minutely.data[59].time * 1000),"h:mm tt")+"</td></tr><tr><td colspan='3' class='center-text'><canvas id='minutelyCanvas' class='border' width='"+minutelyCanvasWidth+"' height='"+(minutelyCanvasHeight+minutelyCanvasTitleHeight)+"'></canvas></td></tr></table>";
